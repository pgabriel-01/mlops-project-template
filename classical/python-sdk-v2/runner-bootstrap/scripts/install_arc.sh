...
az aks command invoke --resource-group "$RESOURCE_GROUP" --name "$AKS_NAME" \
  --command "set -e; kubectl delete pod arc-image-pull-check --ignore-not-found; kubectl run arc-image-pull-check --image='$ARC_RUNNER_IMAGE' --restart=Never --command -- sh -ec 'git --version && curl --version && python3 --version && az version'; kubectl wait --for=jsonpath='{.status.phase}'=Succeeded pod/arc-image-pull-check --timeout=300s; kubectl logs arc-image-pull-check; kubectl delete pod arc-image-pull-check" \
  >/dev/null
